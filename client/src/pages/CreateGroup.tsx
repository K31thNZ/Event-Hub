...
export function CreateGroup() {
  ...
  const autoSlug = (n: string) =>
    n.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "").slice(0, 40);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiRequest("POST", "/api/groups", {
        name, slug, description, category, membershipType,
      ...
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4">
        ...
        <div className="text-center">
          ...
          <p className="text-muted-foreground max-w-sm">
            Groups are available to premium members. Upgrade your membership to create a group.
          </p>
        </div>
        <Button asChild className="rounded-full">
          <a href="/profile">View membership options</a>
        </Button>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <Label>
                    ...
                    <span className="text-muted-foreground font-normal text-xs">
                      — expatevents.org/groups/<strong>{slug || "your-group"}</strong>
                    </span>
                  </Label>
                  <Input
                    value={slug}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`p-4 rounded-2xl border-2 text-left transition-colors ${
                      membershipType === "open" ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"
                    }`}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setMembershipType("invite_only")}
                    className={`p-4 rounded-2xl border-2 text-left transition-colors ${
                      membershipType === "invite_only" ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"
                    }`}
                  >
            ...
            <Card className="rounded-3xl border-border/60 shadow-lg overflow-hidden">
              <div className="bg-primary/5 px-8 py-4 border-b border-border/50">
                <h2 className="text-xl font-bold font-display">Branding <span className="font-normal text-muted-foreground text-base">(optional)</span></h2>
              </div>
              <CardContent className="p-8 space-y-4">
                <div className="space-y-2">
