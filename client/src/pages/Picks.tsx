...
import { Sparkles, Calendar, MapPin, ArrowRight, Star } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import type { CuratorPickWithEvents } from "@shared/schema";
...
export default function Picks() {
  const { data: pick, isLoading } = useQuery<CuratorPickWithEvents | null>({
    queryKey: ["/api/picks"],
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-6 px-4">
        ...
        <div className="text-center">
          <h1 className="text-3xl font-display font-bold mb-2">Picks of the Week</h1>
          <p className="text-muted-foreground max-w-sm">
            Our curators are preparing this week's recommendations. Check back soon.
          </p>
        </div>
  ...
  const weekLabel = format(new Date(pick.weekOf), "'Week of' MMMM d, yyyy");
    ...
    <div className="min-h-screen bg-background pb-24">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-muted-foreground text-lg max-w-xl mx-auto">
              Hand-curated events chosen by our community specialists.
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <motion.div
          ...
          className="bg-card border border-border rounded-3xl p-6 md:p-8 flex flex-col sm:flex-row gap-5"
        >
          <Avatar className="h-16 w-16 shrink-0">
            <AvatarImage src={pick.curatorAvatarUrl ?? ""} />
            <AvatarFallback className="bg-primary/10 text-primary text-xl font-bold">
              {pick.curatorName.substring(0, 2).toUpperCase()}
            </AvatarFallback>
          </Avatar>
          <div className="flex-1">
            <div className="flex items-center gap-3 flex-wrap mb-2">
              <p className="font-display font-bold text-xl">{pick.curatorName}</p>
              <Badge variant="secondary" className="gap-1 rounded-full">
                <Star className="w-3 h-3 text-primary" />
                {pick.curatorSpecialty} curator
              </Badge>
            </div>
            <p className="text-muted-foreground leading-relaxed">{pick.intro}</p>
          </div>
        </motion.div>

        {/* ── Picked events ────────────────────────────────────────────── */}
        <div className="space-y-6">
          {pick.events.map((event, i) => (
            <motion.div
              key={event.id}
              ...
              <Link href={`/events/${event.id}`}>
                <div className="group bg-card border border-border rounded-3xl overflow-hidden hover:shadow-xl hover:shadow-primary/5 transition-all hover:-translate-y-0.5 cursor-pointer flex flex-col md:flex-row">
                  ...
                  <div className="hidden md:flex w-16 shrink-0 bg-primary/5 items-center justify-center border-r border-border">
                    <span className="font-display font-bold text-2xl text-primary/40">
        ...
        <div className="text-center pt-8 border-t border-border">
          <p className="text-muted-foreground mb-4">Looking for more? Browse everything happening this week.</p>
