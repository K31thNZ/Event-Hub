...
// All group-related API routes for Event-Hub.
// Register with: registerGroupRoutes(app) in server/routes.ts

import type { Express, Request, Response } from "express";
import { db } from "./db";
import { groups, groupMembers, events, ticketTypes, users } from "@shared/schema";
...
// ── Resolve group membership for the requesting user ─────────────────────
async function getMembership(groupId: number, userId: number | undefined) {
...
// ── Build recurring event instances ───────────────────────────────────────
...
export function registerGroupRoutes(app: Express) {
  ...
  // ── GET /api/groups ────────────────────────────────────────────────────
  // Public — list all active public groups with member counts.
  app.get("/api/groups", async (req, res) => {
    try {
      ...
      // Attach current user's membership role
  ...
  // ── GET /api/groups/:slug ──────────────────────────────────────────────
  // Public — group profile with members and upcoming events.
  app.get("/api/groups/:slug", async (req, res) => {
    try {
      ...
      const isModOrOwner = ["owner", "moderator"].includes(membership?.role ?? "");

      // Fetch members (show all if public, limited if invite-only)
      ...
      // Fetch events (include private only if member)
      const eventQuery = db.select().from(events)
        .where(and(
      ...
      const groupEvents = await eventQuery;
      ...
      res.json({
        ...group,
        members: group.membershipType === "open" || isActiveMember ? members : members.filter(m => ["owner","moderator"].includes(m.role)),
  ...
  // ── POST /api/groups ───────────────────────────────────────────────────
  // Create a new group. Requires premium+ membership.
  app.post("/api/groups", requireAuth, async (req, res) => {
    try {
      ...
      // Check they don't already own a group
      const [existing] = await db.select().from(groups)
        .where(and(eq(groups.ownerUserId, user.id), ne(groups.status, "suspended")));
      if (existing) {
  ...
  // Soft-delete (suspend) a group. Owner or admin only.
  ...
  // Join a public/open group, or request to join an invite-only group.
  ...
  // Owner adds a moderator (user must already be a member).
  app.post("/api/groups/:id/moderators", requireAuth, async (req, res) => {
    try {
      const groupId = parseInt(req.params.id);
      const membership = await getMembership(groupId, req.session.userId);
      if (membership?.role !== "owner") {
  ...
  app.delete("/api/groups/:id/moderators/:userId", requireAuth, async (req, res) => {
    try {
      const groupId = parseInt(req.params.id);
      const membership = await getMembership(groupId, req.session.userId);
      if (membership?.role !== "owner") {
      ...
      const targetMembership = await getMembership(groupId, parseInt(req.params.userId));
  ...
  // Approve pending request, or ban a member. Owner/mod only.
  app.patch("/api/groups/:id/members/:userId", requireAuth, async (req, res) => {
    try {
      ...
      const target = await getMembership(groupId, parseInt(req.params.userId));
  ...
  // Create an event in the group's name. Owner/mod only.
  // Handles recurring event generation.
  app.post("/api/groups/:id/events", requireAuth, async (req, res) => {
    try {
      ...
      // Insert ticket types for base event
      ...
      if (recurrence) {
        ...
        for (const instanceDate of futureDates) {
          ...
          // Each instance gets the same ticket types
          if (ticketTypesInput?.length > 0) {
            await db.insert(ticketTypes).values(
  ...
  // ── GET /api/groups/my ────────────────────────────────────────────────
  // Returns groups the current user owns or is a member of.
  ...
  // Owner invites a user to an invite-only group by userId.
  ...
  app.post("/api/groups/:id/accept-invite", requireAuth, async (req, res) => {
    try {
      const groupId = parseInt(req.params.id);
      const membership = await getMembership(groupId, req.session.userId);
